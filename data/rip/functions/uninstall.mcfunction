scoreboard objectives remove lastDeath
scoreboard objectives remove RIP_Death
scoreboard objectives remove RIP_Uuid0
scoreboard objectives remove RIP_Uuid1
scoreboard objectives remove RIP_Uuid2
scoreboard objectives remove RIP_Uuid3
scoreboard objectives remove RIP_DeathX
scoreboard objectives remove RIP_DeathY
scoreboard objectives remove RIP_DeathZ
scoreboard objectives remove RIP_Dimension

function fktool:uninstall

datapack disable "file/RIP"
datapack disable "file/RIP-master"

reload
